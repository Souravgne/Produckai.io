...
USING (
  ...
  EXISTS (
    SELECT 1 FROM product_areas WHERE id = themes.product_area_id AND user_id = auth.uid()
  )
)
WITH CHECK (
...
-- Create single insights policy with simplified access checks
...
USING (
  ...
  EXISTS (
    ...
    AND (
      ...
      EXISTS (
        SELECT 1 FROM product_areas WHERE id = t.product_area_id AND user_id = auth.uid()
      )
    )
  )
)
