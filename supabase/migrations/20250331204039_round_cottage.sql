...
DROP POLICY IF EXISTS "insights_access_final_v11" ON insights;
DROP POLICY IF EXISTS "insight_themes_access_final_v11" ON insight_themes;
DROP POLICY IF EXISTS "insight_customers_access_final_v11" ON insight_customers;

-- Create maximally simplified insights policy
CREATE POLICY "insights_access_final_v12"
ON insights
...
CREATE POLICY "insight_themes_access_final_v12"
ON insight_themes
AS PERMISSIVE FOR ALL
TO authenticated
USING (
  ...
  EXISTS (
    SELECT 1 FROM insights WHERE id = insight_themes.insight_id AND created_by = auth.uid()
  )
);

-- Create simplified insight_customers policy
CREATE POLICY "insight_customers_access_final_v12"
...
USING (
  ...
  -- 1. Created the associated insight
  EXISTS (
    SELECT 1 FROM insights WHERE id = insight_customers.insight_id AND created_by = auth.uid()
  )
);
