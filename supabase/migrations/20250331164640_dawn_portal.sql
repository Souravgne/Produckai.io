...
DROP POLICY IF EXISTS "insights_access_v8" ON insights;
DROP POLICY IF EXISTS "insight_themes_access_v7" ON insight_themes;
DROP POLICY IF EXISTS "insight_customers_access_v7" ON insight_customers;

-- Create simplified insights policy that allows viewing insights through themes
CREATE POLICY "insights_access_v9"
ON insights
...
USING (
  ...
  -- 2. Has access to associated theme's product area
...
CREATE POLICY "insight_themes_access_v8"
ON insight_themes
AS PERMISSIVE FOR ALL
TO authenticated
USING (
...
CREATE POLICY "insight_customers_access_v8"
